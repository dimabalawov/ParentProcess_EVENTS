...
void ParentProcessDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    if (IDC_BUTTON1 == id)
    {
        HANDLE hEvent = CreateEvent(NULL, TRUE, TRUE, TEXT("{A59FBCB4-154E-49E6-B0C9-0AB3A60942D7}"));
        if (hEvent == NULL)
        {
        ...
        if (!CreateProcess(NULL, filename, NULL, NULL, FALSE, 0, NULL, NULL, &st, &pr))
        {
            MessageAboutError(GetLastError());
            CloseHandle(hEvent);
            return;
        ...
        st.cb = sizeof(st);
        wsprintf(filename, TEXT("%s"), TEXT("Standard Dialog.exe"));
        if (!CreateProcess(NULL, filename, NULL, NULL, FALSE, 0, NULL, NULL, &st, &pr))
        {
            MessageAboutError(GetLastError());
            CloseHandle(hEvent);
